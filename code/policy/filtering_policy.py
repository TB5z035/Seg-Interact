...
def highest_loss_filtering(args, dataset_path: str, epoch: int):
    ...
    for scene in tqdm(scenes):
        ...
        if osp.exists(scene_loss_path) and osp.exists(scene_labels_path) and osp.exists(prev_updated_path):
            ...
            #print(prev_updated_labels[existing_label_indices])
            #print(scene_losses[existing_label_indices])
...
def random_filtering(args, dataset_path: str, epoch: int):
    ...
    for scene in scenes:
        ...
        if osp.exists(scene_loss_path) and osp.exists(scene_labels_path):
            ...
            ) >= args.update_points_num, f'exceeded max possible number of points to update: {args.update_points_num} > {len(scene_predictions)-existing_num}'
