...
from .sp_transforms import NAGSelectByKey, NAGRemoveKeys, SampleXYTiling, \
...
def sp_init(cfg_dm, dataset_class: object):
    for key_name in cfg_dm.keys():
    ...
    save_processed_root = cfg_dm.save_processed_root
    ignore_label = cfg_dm.ignore_label
    cfg_dict = cfg_dm.__dict__
    cfg_dict.pop('data_dir', None)
    cfg_dict.pop('pre_transform', None)

    return dataset_class(root=data_dir,
                         pre_transform=pre_transform,
                         save_processed_root=save_processed_root,
                         ignore_label=ignore_label,
                         **cfg_dict)


class SuperpointBase(InMemoryDataset):
    ...
    def __init__(
        ...
        self.save_processed_root = save_processed_root
        self._stage = stage
    ...
    def _process_single_cloud(self, cloud_path):
        ...
        if self.save_processed_root is not None:
