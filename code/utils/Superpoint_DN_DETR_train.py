...
def train(local_rank=0, world_size=1, args=None):
    ...
    if args.train_from:
        logger.info(f"Load pretraining from {args.train_from}")
        ckpt = torch.load(args.train_from, map_location='cpu')
        network.load_state_dict(ckpt['network'], strict=False)
    ...
    # elif criterion == 'ChamferDistacne':
    #     from ..sp_dependencies.chamfer3D import dist_chamfer_3D
    #     criterion = dist_chamfer_3D.chamfer_3DDist()
