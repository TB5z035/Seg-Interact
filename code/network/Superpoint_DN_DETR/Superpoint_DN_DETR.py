...
from ...utils.Superpoint_DN_DETR import box_ops
from ...utils.Superpoint_DN_DETR.DN_misc import (NestedTensor, nested_tensor_from_tensor_list,
...
@register_network('Superpoint_DN_DETR')
class Superpoint_DN_DETR(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert self.query_dim == 2
        self.hidden_dim = self.token_embed_dim
        ...
        self.label_enc = nn.Embedding(self.num_classes + 1, self.hidden_dim - 1)  # leave one dim for indicator
        
        if self.bbox_embed_diff_each_layer:
            self.bbox_embed = nn.ModuleList([MLP(self.hidden_dim, self.hidden_dim, 4, 3) for i in range(6)])
        else:
            self.bbox_embed = MLP(self.hidden_dim, self.hidden_dim, 4, 3)
        
        self.refpoint_embed = nn.Embedding(self.num_queries, self.query_dim)
        self.random_refpoints_xy = self.random_refpoints_xy
...
def build_other_dn_func(config):
    # the `num_classes` naming here is somewhat misleading.
    # it indeed corresponds to `max_obj_id + 1`, where max_obj_id
    # is the maximum id for a class in your dataset.

    if config.masks:
        model = DETRsegm(model, freeze_detr=(config.frozen_weights is not None))
    matcher = build_matcher(config)
    weight_dict = {'loss_ce': config.cls_loss_coef, 'loss_bbox': config.bbox_loss_coef}
    weight_dict['loss_giou'] = config.giou_loss_coef
    # dn loss
    if config.use_dn:
        weight_dict['tgt_loss_ce'] = config.cls_loss_coef
        weight_dict['tgt_loss_bbox'] = config.bbox_loss_coef
        weight_dict['tgt_loss_giou'] = config.giou_loss_coef
    ...
    if config.masks:
        losses += ["masks"]
    criterion = SetCriterion(config['num_classes'], matcher=matcher, weight_dict=weight_dict,
                             focal_alpha=config.focal_alpha, losses=losses)

    postprocessors = {'bbox': PostProcess(num_select=config.num_select)}
    if config.masks:
        postprocessors['segm'] = PostProcessSegm()
        if config.dataset_file == "coco_panoptic":
