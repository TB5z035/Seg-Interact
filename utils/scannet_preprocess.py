...
def extract_and_save_labels(scene_path: str) -> None:
    """
    Extract the labels from a scene specified by scene_path, and save to {scene_id}_labels.npy in the same folder.
    """

    if [i for i in os.listdir(scene_path) if i.endswith('_labels.npy')]:
    ...
    np.save(os.path.join(scene_path, f'{scene_id}_labels.npy'), np.array(label_plydata['vertex']['label']))


def extract_and_save_scene(scene_path: str) -> None:
    if [i for i in os.listdir(scene_path) if i.endswith('_scene.npy')]:
    ...
    np.save(
        os.path.join(scene_path, f'{scene_id}_scene.npy'), {
            'coords':
                np.stack(
                    [
...
def preprocess_scannet(scannet_path: str, start=0, split=1) -> None:
    """
    ...
    for scene in tqdm(sorted(scenes)):
        extract_and_save_scene(scene)
        extract_and_save_labels(scene)
...
if __name__ == '__main__':
    ...
    pool.starmap(preprocess_scannet, [(args.scannet_path, i, args.process) for i in range(args.process)])
